...
//           documentId: document,
//           uid: user?.uid || userData?.uid || "unknown_uid",
//           logo: user?.photoURL || avatar,
//           name: user?.displayName || userData?.name || "Anonymous",
//           email: user?.email || userData?.email || "unknown_email",
  ...
  useEffect(() => {
    if (window.cloudinary) {
      window.myWidget = window.cloudinary.createUploadWidget(
        {
          cloudName: "dihhgkyxr",
          uploadPreset: "expertizo_hackathon_media_app", 
        },
        (error, result) => {
    ...
    <div className="flex flex-col items-center">
      <div className="flex flex-col py-4 w-full bg-white rounded-3xl shadow-lg">
          ...
          <form className="w-full" onSubmit={handleSubmitPost}>
            <div className="flex justify-between items-center">
              <input
                ...
                className="outline-none w-full bg-white rounded-md ml-4"
                ref={text}
              />
              <div className="mx-4">
                {image ? (
                  <img className="h-24 rounded-xl" src={image} alt="preview" />
                ) : (
                  <img className="h-24 rounded-xl" src={addImage} alt="default" />
                )}
              </div>
        ...
        <div className="flex justify-around items-center pt-4">
          ...
          >
            <img className="h-10 mr-4" src={addImage} alt="Upload" />
            Upload Image
          </button>
          <div className="flex items-center">
      ...
      <div className="flex flex-col py-4 w-full">
        {state?.error ? (
          ...
          state?.posts?.map((post, index) => (
            <PostCard key={index} {...post} timestamp={new Date(post?.timestamp?.toDate()).toUTCString()} />
          ))
        )}
      </div>
