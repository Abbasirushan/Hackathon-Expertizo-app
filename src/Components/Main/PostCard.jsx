...
const PostCard = ({ uid, id, logo, name, email, text, image, timestamp }) => {
  ...
  const likesRef = id ? doc(collection(db, "posts", id, "likes")) : null;
  const likesCollection = id ? collection(db, "posts", id, "likes") : null;
  const singlePostDocument = id ? doc(db, "posts", id) : null;
